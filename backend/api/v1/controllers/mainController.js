...
const history_csv = path.resolve(path.join('../backend/', 'history.csv'));
...
exports.createUser = (req, res, next) => {
	...
	newUser.save((err) => {
		if (err) {
			...
			return res
				.status(500)
				.send({ error: 'Internal Error, Failed to create record...' });
		}
	});
	return res.status(200).send({});
};
...
exports.fetchUser = (req, res, next) => {
	user.find({ username: req.body.username }, (err, users) => {
		...
		userData = users[0];
		// console.log(userData.imageString.substring(1, 10));
		...
		return res.status(200).send({
			...
			imageString: userData.imageString,
		});
	});
};
...
exports.storeBook = (req, res, next) => {
	...
	newBook.save((err) => {
		if (err) {
			...
			return res
				.status(500)
				.send({ error: 'Interal Error, Failed to create record...' });
		}
	});
	return res.status(200).send({});
};
...
exports.deleteImage = (req, res, next) => {
	...
	image.findOne(
		{ imagetype: req.body.imageType, value: req.body.value },
		(err, imageData) => {
			if (err) {
				console.log('Error :,\n', err);
				return res.status(500).send({ error: 'Internal Error...' });
			...
			if (req.body.imageType == 'user') {
				...
				targetPath =
					books_dir + imageData.value + '.' + imageData.imageformat;
			}
			...
			try {
				...
				image.deleteOne(
					...
					(err, users) => {
						if (err) {
							console.log(
								'Error in deleting image record:,\n',
								err
							);
							return res
								.status(500)
...
exports.searchBooks = (req, res, next) => {
	...
	return book.find({ title: new RegExp(search_query, 'i') }, (err, books) => {
		...
		return res.status(200).send(books.splice(0, numBooks));
	});
};

exports.fetchBook = (req, res, next) => {
	let search_query = req.query.id;
	return book.findOne({ isbn13: search_query }, (err, returned_book) => {
		if (err) {
			console.log('Error :,\n', err);
			return res.status(500).send({ error: 'Interal Error...' });
		}
